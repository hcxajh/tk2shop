...
for (let i = 0; i < mainImgs.length; i++) {
  ...
  const r = await downloadImg(cleanUrl, path.join(imgDir, file));
  const sz = fs.existsSync(path.join(imgDir,file)) ? fs.statSync(path.join(imgDir,file)).size : 0;
  console.log(`    -> ${r}, size=${sz}`);
}
